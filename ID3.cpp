...
double get_initial_shang(vector<data_type> d){//index  = location of tag;num_split = 分类个数 
	...
	if(sum == 0 || sum == d.size())
		...
		result = -(sum/d.size())*log2(sum/d.size()) - 
		(((double)d.size()-sum)/d.size())*log2(((double)d.size()-sum)/d.size());
	return result;	
}

//返回分类后的所有数据集以及熵 同时为了方便计算 返回这个标签的位置以及标签的值 
...
//返回每次分类的边界依据 尝试了两种方法 等距离分类 以及按每两个不相同数据的中点来分类 
